...
const ContextProviderLibros = ({children}) => {
    ...
    const storeLibros = async (libro) => {
        ...
        if(libro.id) {
            try {
                const {data} = await axios.put(`${process.env.URL_BACKEND}/api/libros/${libro.id}`, libro, configuration);
            ...
            try {
                const {data} = await axios.post(`${process.env.URL_BACKEND}/api/libros`, libro, configuration);
                console.log(data);
    ...
    useEffect(() => {
        const getLibros = async () => {
            try {
                ...
                const {data} = await axios.get(`${process.env.URL_BACKEND}/api/libros`, configuration);
                setLibros([]);
                setLibros(data);
    ...
    const deleteLibro = async (libro) => {
        ...
        if(authDelete) {
            try {
                const {data} = await axios.delete(`${process.env.URL_BACKEND}/api/libros/${libro._id}`, configuration);
